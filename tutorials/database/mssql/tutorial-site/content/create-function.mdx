...
MS SQL user-defined functions are of 2 types: Scalar and Tabular-Valued based on the type of result set each returns.

A Scalar function accepts one ore more parameters and returns a single value.

A Table-Valued function takes one input parameter and returns the columns of the table.

> Requires `CREATE FUNCTION` permission in the database and `ALTER` permission on the schema in which the functions is being created. [ as referred in the Microsoft Docs ]
