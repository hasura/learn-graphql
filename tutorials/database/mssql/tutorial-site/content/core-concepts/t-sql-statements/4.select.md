...
A derived column is based on the calculation results on other column(s).
...
The `LIKE` clause retrieve only the rows that matches the search pattern (regular expression).
...
```SQL
-- Retrieve all the roles, where Job Title not like the given string.
SELECT DISTINCT JobTitle FROM HumanResources.Employee
...
```SQL
--Rows with OrganizationLevel that does not include 2,3, AND 4.
SELECT DISTINCT OrganizationLevel FROM HumanResources.Employee
...
Is used along with `GROUP BY` clause to limit the result set obtained by the Group by clause.
...
Use `SELECT INTO` statement to create another (backup) table from an existing table.
