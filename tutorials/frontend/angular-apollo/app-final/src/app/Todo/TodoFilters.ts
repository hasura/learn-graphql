...
export class TodoFilters implements OnChanges {
  ...
  @Output() filterResultsFn: EventEmitter<any> = new EventEmitter();
  @Output() clearCompletedFn: EventEmitter<any> = new EventEmitter();
  ...
  filterResultsHandler(filter) {
    this.filterResultsFn.emit({ event, filter });
  }
}
